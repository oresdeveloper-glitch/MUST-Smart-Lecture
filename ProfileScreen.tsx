...
export default function ProfileScreen() {
  const { user, badges, attendanceRecords } = useApp();
    ...
    <div className="max-w-2xl mx-auto space-y-6 page-enter-stagger relative">
      ...
      <div className="premium-card p-6 text-center">
        <div className="relative inline-flex">
          <div className="w-24 h-24 rounded-full must-shield-bg flex items-center justify-center text-white text-3xl font-bold shadow-xl shadow-[#0a1628]/20 ring-4 ring-[var(--must-gold)]/20">
            {user?.name?.charAt(0) || 'U'}
          </div>
          <button className="absolute bottom-0 right-0 p-1.5 rounded-full bg-[var(--must-gold)] text-[#0a1628] hover:bg-[var(--must-gold-light)] transition-colors shadow-lg">
        ...
        <button className="mt-3 flex items-center gap-1.5 mx-auto text-sm font-semibold text-[var(--must-gold)] hover:underline">
      ...
      <div className="premium-card overflow-hidden">
        ...
        <div className="divide-y divide-slate-100 dark:divide-slate-800">
          {[
            ...
            { icon: Shield, label: 'ID', value: user?.studentId || user?.staffId || '—' },
            { icon: Calendar, label: 'Member Since', value: user?.createdAt ? new Date(user.createdAt).toLocaleDateString('en-US', { month: 'long', year: 'numeric' }) : '—' },
          ].map((item, i) => (
